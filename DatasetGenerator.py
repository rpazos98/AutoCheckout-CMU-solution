    ...
    for product_sku in sku_to_png_list.keys():
        ...
        while sku_example_count < SKU_EXAMPLE_MIN:
            if random.choice([True, False]):
                ...
                transformed_product = Image.fromarray(np.uint8(transformed_product)).convert('RGB')
                ...
                x, y = random.randint(0, max(0, background.size[0] - product.size[0])), \
                ...
                writer.addObject(product_sku, xmin=xmin / 3, ymin=ymin / 3, xmax=xmax / 3, ymax=ymax / 3)
...
def create_train_and_test(generated_files, train_size):
    ...
    test_list = generated_files[int(len(generated_files) * train_size): int(len(generated_files))]
