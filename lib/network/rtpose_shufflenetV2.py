...
class BasicBlock(nn.Module):
    ...
    def generate_caffe_prototxt(self, caffe_net, layer):
        if self.stride == 1:
            layer_x1, layer_x2 = L.Slice(
                layer, ntop=2, axis=1, slice_point=[self.in_channels // 2]
            )
...
if __name__ == "__main__":
    ...
    if args.test is None:
        ...
        img = torchvision.transforms.functional.resize(
            img, (args.image_hw, args.image_hw)
        )
    ...
    if args.load_pytorch is not None:
        net.load_state_dict(
            torch.load(args.load_pytorch, map_location=lambda storage, loc: storage)
        )
