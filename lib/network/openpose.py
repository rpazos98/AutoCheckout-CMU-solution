...
class StageBlock(nn.Module):
    ...
    def __init__(self, in_channels, inner_channels, innerout_channels, out_channels):
        ...
        self.Mconv6 = ConvBlock(
            inner_channels * 3, innerout_channels, kernel_size=1, stride=1, padding=0
        )
        self.Mconv7 = nn.Conv2d(
            in_channels=innerout_channels,
            out_channels=out_channels,
            kernel_size=1,
            stride=1,
            padding=0,
...
class OpenPose_Model(nn.Module):
    def __init__(
        self, l2_stages=4, l1_stages=2, paf_out_channels=14, heat_out_channels=9
    ):
        """
        ...
        self.l2_stages = nn.ModuleList(
            [
                StageBlock(
                    in_channels=L2_IN_CHS[i],
                    inner_channels=L2_INNER_CHS[i],
                    innerout_channels=L2_INNEROUT_CHS[i],
                    out_channels=L2_OUT_CHS[i],
                )
        ...
        self.l1_stages = nn.ModuleList(
            [
                StageBlock(
                    in_channels=L1_IN_CHS[i],
                    inner_channels=L1_INNER_CHS[i],
                    innerout_channels=L1_INNEROUT_CHS[i],
                    out_channels=L1_OUT_CHS[i],
                )
    ...
    def forward(self, x):
        ...
        return [
            (paf_ret[-2], heat_ret[-2]),
            (paf_ret[-1], heat_ret[-1]),
        ], saved_for_loss
    ...
    def init_w_pretrained_weights(
        ...
        conv_idxs = [
            ...
            if "conv" in d["name"]
            and "split" not in d["name"]
            and "concat" not in d["name"]
        ]
        prelu_idxs = [
            i
            for i, d in enumerate(weights)
...
def use_vgg(model):
    ...
    for i in range(20):
        weights_load[list(model.state_dict().keys())[i]] = vgg_state_dict[
            list(vgg_keys)[i]
        ]
