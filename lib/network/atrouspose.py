...
class ASPP_ASP(nn.Module):
    def __init__(self, in_, out_=16):
        ...
        self.conv_1x1_1 = nn.Conv2d(
            in_, 128, kernel_size=3, stride=1, padding=1, dilation=1
        )
        self.bn_conv_1x1_1 = nn.BatchNorm2d(128)

        self.conv_3x3_1 = nn.Conv2d(
            in_, 128, kernel_size=3, stride=1, padding=4, dilation=4
        )
        self.bn_conv_3x3_1 = nn.BatchNorm2d(128)

        self.conv_3x3_2 = nn.Conv2d(
            in_, 128, kernel_size=3, stride=1, padding=8, dilation=8
        )
        self.bn_conv_3x3_2 = nn.BatchNorm2d(128)

        self.conv_3x3_3 = nn.Conv2d(
            in_, 128, kernel_size=3, stride=1, padding=16, dilation=16
        )
    ...
    def forward(self, feature_map):
        ...
        add4 = add3 + out_3x3_3
        out = F.relu(
            self.bn_out(torch.cat([add1, add2, add3, add4], 1))
...
class AtrousPose(nn.Module):
    def __init__(self, paf_out_channels=38, heat_out_channels=19):
        ...
        self.h1 = nn.Sequential(
            ...
            conv(
                512, heat_out_channels, kernel_size=1, padding=0, bn=False, relu=False
            ),
        )
