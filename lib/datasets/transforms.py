...
class Normalize(Preprocess):
    ...
    def normalize_annotations(anns):
        ...
        for ann in anns:
            ann["keypoints"] = np.asarray(ann["keypoints"], dtype=np.float32).reshape(
                -1, 3
            )
...
class Compose(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        ...
        image, anns, meta = reduce(
            lambda md_i_mm, f: f(*md_i_mm), augmentations, (image, anns, meta)
        )
        return image, anns, meta
...
class RescaleRelative(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        ...
        if isinstance(self.scale_range, tuple):
            scale_factor = self.scale_range[0] + torch.rand(1).item() * (
...
class RescaleAbsolute(Preprocess):
    ...
    def scale(self, image, anns):
        ...
        if isinstance(this_long_edge, (tuple, list)):
            this_long_edge = int(
                torch.randint(this_long_edge[0], this_long_edge[1], (1,)).item()
            )
        ...
        self.log.debug(
            "before resize = (%f, %f), scale factor = %f, after = %s",
            w,
            h,
            s,
            image.size,
...
class CenterPad(Preprocess):
    ...
    def center_pad(self, image, anns):
        ...
        # pad image
        image = torchvision.transforms.functional.pad(image, ltrb, fill=(124, 116, 104))
...
class RandomRotate(Preprocess):
    ...
    def __call__(self, image, anns, meta):
        ...
        dice = random.random()
        degree = (dice - 0.5) * 2 * self.max_rotate_degree  # degree [-40,40]

        img_rot, R = self.rotate_bound(
            np.asarray(image), np.copy(degree), (128, 128, 128)
        )
