...
class VizUtils:
    ...
    def __init__(self, events, timestamps, db_name, weight_shelf_mean, weight_shelf_std, book_keeper):
        self.events = events
    ...
    def __graph_event_to_position_3d(self, db_name, type):
        ...
        events = px.scatter_3d(df, x='x', y='y', z="z",
                               ...
                               hover_data=['x', 'z', 'datetime_begin', 'weight_delta', "event_type", "product_name",
                                           "product_quantity", "product_weight"], range_x=[0, 4.5], range_y=[0, 2],
                               range_z=[0, 2])
        camera = dict(
            up=dict(x=0, y=0, z=1),
        ...
        for gondola_id in range(1, NUM_GONDOLA):
            for shelf_id in range(1, NUM_SHELF):
                for plate_id in range(1, NUM_PLATE):
                    coordinates = self.book_keeper.get3DCoordinatesForPlate(gondola_id, shelf_id, plate_id)
                    ...
                    tags.append("GONDOLA {}, SHELF {}, PLATE {}".format(str(gondola_id), str(shelf_id), str(plate_id)))
        data = {"x": xs, "y": ys, "z": zs, "tags": tags, "gondola": gondolas, "shelves": shelves, "plates": plates}
        return pd.DataFrame(data)

    def __graph_plates(self, plates_coordinates, db_name, type):
        plotly_fig = px.scatter_3d(plates_coordinates, x='x', y='y', z="z", range_x=[0, 4.5], range_y=[0, 2],
                                   color='shelves', symbol="gondola", range_z=[0, 2],
